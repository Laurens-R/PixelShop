...
import { boxBlur } from '@/graphicspipeline/webgpu/compute/filterCompute'
import { selectionStore } from '@/core/store/selectionStore'
import type { CanvasHandle } from '@/ux/main/Canvas/canvasHandle'
import { ToolWindow } from '@/ux'
import styles from './BoxBlurDialog.module.scss'
...
const BlurIcon = (): React.JSX.Element => (
  <svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.2" aria-hidden="true">
    ...
    <rect x="4" y="4" width="4" height="4" rx="0.5" opacity="0.4" />
  </svg>
)

// ─── Props ────────────────────────────────────────────────────────────────────

export interface BoxBlurDialogProps {
  isOpen:          boolean
  onClose:         () => void
...
export function BoxBlurDialog({
  ...
  canvasWidth,
  canvasHeight,
}: BoxBlurDialogProps): React.JSX.Element | null {
  ...
  const runPreview = useCallback(async (r: number): Promise<void> => {
    ...
    setIsBusy(true)
    try {
      const blurred  = await boxBlur(original.slice(), canvasWidth, canvasHeight, r)
  ...
  const handleApply = useCallback(async (): Promise<void> => {
    ...
    setIsBusy(true)
    try {
      const blurred  = await boxBlur(original.slice(), canvasWidth, canvasHeight, radius)
      const composed = applySelectionComposite(blurred, original, selectionMaskRef.current)
      handle.writeLayerPixels(activeLayerId, composed)
      captureHistory('Box Blur')
      onClose()
    } catch (err) {
      console.error('[BoxBlur] Apply failed:', err)
  ...
  return (
    <ToolWindow title="Box Blur" icon={<BlurIcon />} onClose={handleCancel} width={284}>
